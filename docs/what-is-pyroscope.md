...
When ingestion volume exceeds single-node capacity (>100 services, HA requirements, or
...
```mermaid
graph LR
    subgraph "Phase 1 — Prove Value"
        VM["VM Monolith<br/>1 process<br/>2 GB RAM, 1 CPU"]
        Agent1["JVM Agents<br/>5-20 services"]
        Agent1 -->|profiles| VM
    end

    subgraph "Phase 2 — Scale"
        OCP["Microservices on OCP/K8s<br/>9 components<br/>HA, horizontal scale"]
        Agent2["JVM Agents<br/>100+ services"]
        Agent2 -->|profiles| OCP
    end

    VM -->|"Ingestion exceeds<br/>single-node capacity"| OCP
...
requires no Kubernetes knowledge, and the agent configuration is identical for both phases.
Migration to Phase 2 is a server-side change only — agents continue to push profiles to the
same endpoint.
