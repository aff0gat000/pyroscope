...
| **Phase 2** | Multi-VM monolith with S3-compatible object storage (MinIO, AWS S3, GCS, or Azure Blob), HA via load balancer (F5 VIP) | + 1 VM, S3-compatible object storage, load balancer |
| **Phase 3** | PostgreSQL-backed SORs, v2 BOR functions, microservices mode on OpenShift | + OCP namespace, S3-compatible object storage, PostgreSQL instance |
...
| D3 Infrastructure | 1 VM request + S3-compatible object storage (MinIO, AWS S3, GCS, or Azure Blob) + firewall rules |
| D4 Cost | $0 licensing (+ object storage costs if using cloud S3) |
| D5 Data sovereignty | All data on-premise (MinIO) or in controlled cloud storage |
| D6 Agent overhead | Same agent — 3-8% CPU, 20-40 MB memory |
...
- **Option 3 (microservices on VM):** Complexity of microservices without orchestration benefits. Phase 2 (multi-VM monolith with S3-compatible object storage) provides HA without microservices complexity.
...
| No HA / disaster recovery | Low | Pyroscope is a non-critical observability tool. Profiling gaps during outages are acceptable. Persistent Docker volume survives container restarts. | Phase 2 (multi-VM with S3-compatible object storage) adds HA via VIP failover |
