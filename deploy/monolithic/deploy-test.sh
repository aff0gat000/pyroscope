...
run_deploy() {
    ...
    CONTAINER_NAME="test-pyroscope" \
    IMAGE_NAME="test-pyroscope-server" \
    VOLUME_NAME="test-pyroscope-data" \
    HEALTH_TIMEOUT="4" \
...
# ---- Tests ----------------------------------------------------------------
echo ""
echo "Running vm-deploy.sh tests..."
echo ""
...
assert_output_contains "start prints port"          "14040"       start
assert_output_contains "start prints ingest URL"    "ingest"      start
assert_output_contains "start prints server.address" "pyroscope.server.address" start
teardown
...
echo "storage: {}" > "${LOCAL_SRC}/pyroscope.yaml"
assert_success "start --from-local succeeds" start --from-local "${LOCAL_SRC}"
teardown

setup
LOCAL_SRC="${TEST_TMP}/local-src-repo"
mkdir -p "${LOCAL_SRC}/deploy/monolithic"
echo "FROM scratch" > "${LOCAL_SRC}/deploy/monolithic/Dockerfile"
echo "storage: {}" > "${LOCAL_SRC}/deploy/monolithic/pyroscope.yaml"
