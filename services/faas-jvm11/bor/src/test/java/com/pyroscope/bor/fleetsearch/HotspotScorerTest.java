...
class HotspotScorerTest {
    ...
    void rankHotspots_scoreIsServiceCountTimesMaxPercent() {
        ...
        assertThat(hotspot.getDouble("impactScore")).isEqualTo(40.0);
    ...
    void rankHotspots_sortsByScoreDescending() {
        Map<String, List<JsonObject>> input = new HashMap<>();
        input.put("low", Collections.singletonList(
                new JsonObject().put("app", "svc1").put("selfPercent", 5.0)
        ));
        input.put("high", Arrays.asList(
                new JsonObject().put("app", "svc1").put("selfPercent", 10.0),
    ...
    void rankHotspots_respectsLimit() {
        Map<String, List<JsonObject>> input = new HashMap<>();
        input.put("a", Collections.singletonList(new JsonObject().put("app", "s1").put("selfPercent", 30.0)));
        input.put("b", Collections.singletonList(new JsonObject().put("app", "s1").put("selfPercent", 20.0)));
        input.put("c", Collections.singletonList(new JsonObject().put("app", "s1").put("selfPercent", 10.0)));
    ...
    void rankHotspots_emptyInput_returnsEmpty() {
        var result = HotspotScorer.rankHotspots(Collections.emptyMap(), 10);
