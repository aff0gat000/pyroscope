...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        AbstractVerticle verticle = switch (function) {
            // Phase 1 — standalone, calls Pyroscope directly
            case "ReadPyroscopeTriageAssessment.v1" -> new TriageVerticle(pyroscopeUrl, port);

            // Phase 2 v1 — needs ReadPyroscopeProfile.sor.v1 deployed separately
            case "ReadPyroscopeDiffReport.v1" -> new DiffReportVerticle(profileDataUrl, port);
            case "ReadPyroscopeFleetSearch.v1" -> new FleetSearchVerticle(profileDataUrl, port);

            // Phase 2 v2 — also calls PostgreSQL-backed SORs
            case "ReadPyroscopeTriageAssessment.v2" -> new TriageFullVerticle(
                    profileDataUrl, baselineUrl, historyUrl, port);
            case "ReadPyroscopeDiffReport.v2" -> new DiffReportFullVerticle(
                    profileDataUrl, baselineUrl, historyUrl, port);
            case "ReadPyroscopeFleetSearch.v2" -> new FleetSearchFullVerticle(
