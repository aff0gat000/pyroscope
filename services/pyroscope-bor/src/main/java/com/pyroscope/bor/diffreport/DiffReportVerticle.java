...
public class DiffReportVerticle extends AbstractFunctionVerticle {

    private final String profileDataUrl;
    private SorClient sor;

    public DiffReportVerticle(String profileDataUrl, int port) {
        super(port);
    ...
    private void handleDiff(RoutingContext ctx) {
        ...
        sor.getProfileDiff(appName, type, baselineFrom, baselineTo, from, to, 500)
                .onSuccess(diffData -> {
                    ...
                    List<DiffEntry> diffs = computeDeltas(baselineMap, currentMap, limit);
